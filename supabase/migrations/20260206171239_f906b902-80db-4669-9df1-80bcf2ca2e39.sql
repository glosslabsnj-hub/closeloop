...
CREATE INDEX idx_fleet_drivers_tenant ON public.fleet_drivers(tenant_id);
CREATE INDEX idx_fleet_drivers_status ON public.fleet_drivers(tenant_id, status);
CREATE INDEX idx_fleet_drivers_user ON public.fleet_drivers(user_id) WHERE user_id IS NOT NULL;
...
CREATE INDEX idx_dispatch_jobs_driver ON public.dispatch_jobs(driver_id) WHERE driver_id IS NOT NULL;
CREATE INDEX idx_dispatch_jobs_vehicle ON public.dispatch_jobs(vehicle_id) WHERE vehicle_id IS NOT NULL;
...
CREATE POLICY "Tenant users can view fleet_drivers" ON public.fleet_drivers
  FOR SELECT USING (
    tenant_id IN (SELECT tenant_id FROM public.tenant_users WHERE user_id = auth.uid())
  );

-- Tenant users (owner/staff) can insert drivers
CREATE POLICY "Tenant users can insert fleet_drivers" ON public.fleet_drivers
...
CREATE POLICY "Tenant users can update fleet_drivers" ON public.fleet_drivers
  FOR UPDATE USING (
    tenant_id IN (
      SELECT tenant_id FROM public.tenant_users 
      WHERE user_id = auth.uid() AND role IN ('owner', 'staff')
...
CREATE POLICY "Tenant users can delete fleet_drivers" ON public.fleet_drivers
  FOR DELETE USING (
    tenant_id IN (
      SELECT tenant_id FROM public.tenant_users 
      WHERE user_id = auth.uid() AND role IN ('owner', 'staff')
...
CREATE POLICY "Drivers can view own record" ON public.fleet_drivers
  FOR SELECT USING (user_id = auth.uid());

-- Drivers can update their own record (limited fields via app logic)
CREATE POLICY "Drivers can update own record" ON public.fleet_drivers
  FOR UPDATE USING (user_id = auth.uid());

-- RLS Policies for fleet_vehicles

-- Tenant users can view all vehicles in their tenant
CREATE POLICY "Tenant users can view fleet_vehicles" ON public.fleet_vehicles
  FOR SELECT USING (
    tenant_id IN (SELECT tenant_id FROM public.tenant_users WHERE user_id = auth.uid())
  );

-- Tenant users (owner/staff) can insert vehicles
CREATE POLICY "Tenant users can insert fleet_vehicles" ON public.fleet_vehicles
  FOR INSERT WITH CHECK (
    tenant_id IN (
      SELECT tenant_id FROM public.tenant_users 
      WHERE user_id = auth.uid() AND role IN ('owner', 'staff')
...
CREATE POLICY "Tenant users can update fleet_vehicles" ON public.fleet_vehicles
  FOR UPDATE USING (
    tenant_id IN (
      SELECT tenant_id FROM public.tenant_users 
      WHERE user_id = auth.uid() AND role IN ('owner', 'staff')
...
CREATE POLICY "Tenant users can delete fleet_vehicles" ON public.fleet_vehicles
  FOR DELETE USING (
    tenant_id IN (
      SELECT tenant_id FROM public.tenant_users 
      WHERE user_id = auth.uid() AND role IN ('owner', 'staff')
...
CREATE POLICY "Drivers can view tenant vehicles" ON public.fleet_vehicles
  FOR SELECT USING (
    tenant_id IN (
...
CREATE POLICY "Drivers can update vehicle assignment" ON public.fleet_vehicles
  FOR UPDATE USING (
    tenant_id IN (
      SELECT fd.tenant_id FROM public.fleet_drivers fd WHERE fd.user_id = auth.uid()
    )
