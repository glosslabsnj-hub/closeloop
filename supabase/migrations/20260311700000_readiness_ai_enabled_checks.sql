-- Add AI enabled checks to readiness function
-- After onboarding, ai_enabled and voice_ai_enabled should be true.
-- If they're not, the readiness check should flag it as P0 so guided setup catches it.
-- Date: 2026-03-11
...
DECLARE
  ...
  policies_json jsonb;
  has_real_policies boolean := false;
  -- Notification check variables
  delivery_record record;
  has_notify_phone boolean := false;
  has_notify_email boolean := false;
  has_transfer_number boolean := false;
BEGIN
  -- Fetch tenant data
  ...
  IF has_assistant THEN
    ...
    has_transfer_number := (assistant_record.owner_forward_number IS NOT NULL AND assistant_record.owner_forward_number != '');
  ELSE
  ...
  -- ============================================
  -- AI ENABLED CHECKS (ALL MODES)
  -- These are P0 — if AI isn't enabled, nothing works
  -- ============================================
  IF tenant_record.ai_enabled IS NOT TRUE THEN
    p0_flags := array_append(p0_flags, 'ai_not_enabled');
    recommendations := recommendations || jsonb_build_array(jsonb_build_object(
      'label', 'Enable AI for your business', 'deep_link', '/app/settings'));
  END IF;

  IF has_assistant AND assistant_record.voice_ai_enabled IS NOT TRUE THEN
    p0_flags := array_append(p0_flags, 'voice_ai_disabled');
    recommendations := recommendations || jsonb_build_array(jsonb_build_object(
      'label', 'Enable voice AI so your phone line works', 'deep_link', '/app/settings'));
  END IF;

  -- ============================================
  -- NOTIFICATION CHANNEL CHECKS (ALL MODES)
  -- These don't affect score but block go-live
  -- ============================================
  SELECT * INTO delivery_record FROM universal_delivery_settings WHERE tenant_id = tenant_uuid;
  IF FOUND THEN
    has_notify_phone := (delivery_record.notify_phone IS NOT NULL AND delivery_record.notify_phone != '');
    has_notify_email := (delivery_record.notify_email IS NOT NULL AND delivery_record.notify_email != '');
  END IF;

  IF NOT has_notify_phone THEN
    p0_flags := array_append(p0_flags, 'missing_notification_phone');
    recommendations := recommendations || jsonb_build_array(jsonb_build_object(
      'label', 'Add notification phone for SMS alerts', 'deep_link', '/app/settings?tab=notifications'));
  END IF;

  IF NOT has_notify_email THEN
    p1_flags := array_append(p1_flags, 'missing_notification_email');
    recommendations := recommendations || jsonb_build_array(jsonb_build_object(
      'label', 'Add notification email', 'deep_link', '/app/settings?tab=notifications'));
  END IF;

  IF NOT has_transfer_number THEN
    p0_flags := array_append(p0_flags, 'missing_transfer_number');
    recommendations := recommendations || jsonb_build_array(jsonb_build_object(
      'label', 'Set transfer number so AI can connect callers to you', 'deep_link', '/app/settings?tab=notifications'));
  END IF;
  ...
  -- 4. Policies (10 pts) — require meaningful policy content, not just empty JSON
  policies_json := tenant_record.ai_policies_json;
  IF (tenant_record.cancellation_policy IS NOT NULL AND length(tenant_record.cancellation_policy) > 10) THEN
    has_real_policies := true;
  ELSIF policies_json IS NOT NULL AND jsonb_typeof(policies_json) = 'object' AND policies_json != '{}'::jsonb THEN
    IF (policies_json->>'cancellation_text' IS NOT NULL AND length(policies_json->>'cancellation_text') > 5)
       OR (policies_json->>'deposit_required' IS NOT NULL AND (policies_json->>'deposit_required')::boolean = true)
       OR (policies_json->>'refund_policy' IS NOT NULL AND length(policies_json->>'refund_policy') > 5)
       OR (policies_json->>'no_show_policy' IS NOT NULL AND length(policies_json->>'no_show_policy') > 5)
       OR (policies_json ? 'payment_methods' AND jsonb_array_length(policies_json->'payment_methods') >= 2)
    THEN
      has_real_policies := true;
    END IF;
  END IF;

  IF has_real_policies THEN
    score := score + 10;
  ELSE
  ...
  WHEN 'general' THEN
    ...
    IF has_ai_assistant AND ai_record.greeting_script IS NOT NULL AND length(trim(ai_record.greeting_script)) > 5 THEN
      score := score + 20;
    ELSE
      score := score + 5;
      p1_flags := array_append(p1_flags, 'missing_greeting');
