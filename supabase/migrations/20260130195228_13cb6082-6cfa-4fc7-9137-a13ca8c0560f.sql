...
-- Add readiness cache columns to assistant_settings
ALTER TABLE public.assistant_settings
ADD COLUMN IF NOT EXISTS readiness_score integer DEFAULT 0,
ADD COLUMN IF NOT EXISTS readiness_p0_flags jsonb DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS readiness_p1_flags jsonb DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS readiness_recommendations jsonb DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS readiness_last_computed_at timestamp with time zone;

-- Create the get_ai_readiness RPC function
...
BEGIN
  ...
  ELSIF faqs_count >= 2 THEN
    ...
    recommendations := recommendations || jsonb_build_array(jsonb_build_object(
      ...
      'deep_link', '/app/business-brain#faqs'
    ));
  ELSE
  ...
  WHEN 'service' THEN
    ...
    -- Service pricing configured (10 points) - check if any service has price_type
    ...
    IF assistant_record IS NOT NULL AND assistant_record.ai_booking_mode IS NOT NULL THEN
      score := score + 15;
    ELSE
    ...
    -- Service area (10 points) - optional but recommended for on-site services
  ...
  WHEN 'food' THEN
    ...
    IF food_settings ? 'pickup_enabled' OR food_settings ? 'delivery_enabled' THEN
      ...
      END IF;
    ELSE
      -- Default: assume ordering is enabled if food_settings is empty
      score := score + 10;
    ...
    IF food_settings ? 'special_instructions_enabled' THEN
      ...
      END IF;
    ELSE
      score := score + 10; -- Default to enabled
    END IF;
  ...
  -- ============================================
  WHEN 'medical' THEN
    -- HIPAA mode enabled (20 points) - P0
    ...
    -- Data retention settings exist (15 points) - P0
    IF has_data_retention THEN
      score := score + 15;
      -- Check if properly configured (transcripts/recordings off by default)
  ...
  -- ============================================
  ELSE
    -- General mode has lighter requirements
    -- Just need basic FAQs (already counted) + some configuration
    -- Add 30 bonus points for having assistant settings
    IF assistant_record IS NOT NULL THEN
      score := score + 30;
    END IF;

    -- Add 30 points for having hours (already counted above, so just add remainder)
    score := score + 30;
  END CASE;
...
GRANT EXECUTE ON FUNCTION public.get_ai_readiness(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_ai_readiness(uuid) TO anon;
