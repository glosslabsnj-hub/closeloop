...
CREATE INDEX idx_business_memory_tenant ON public.business_memory(tenant_id);
CREATE INDEX idx_business_memory_location ON public.business_memory(location_id);
CREATE INDEX idx_business_memory_active ON public.business_memory(tenant_id, is_active, confidence_score DESC);
CREATE INDEX idx_business_memory_type ON public.business_memory(tenant_id, memory_type);
...
CREATE TABLE public.business_intent_rules (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
...
CREATE INDEX idx_intent_rules_tenant ON public.business_intent_rules(tenant_id);
CREATE INDEX idx_intent_rules_enabled ON public.business_intent_rules(tenant_id, is_enabled, priority DESC);
CREATE INDEX idx_intent_rules_suggested ON public.business_intent_rules(tenant_id, is_suggested);
...
CREATE TABLE public.tenant_intelligence_settings (
