...
CREATE TABLE IF NOT EXISTS public.agency_lead_searches (
...
DROP POLICY IF EXISTS "Agency users can view own searches" ON public.agency_lead_searches;
...
DROP POLICY IF EXISTS "Agency users can insert own searches" ON public.agency_lead_searches;
...
-- Agency Saved Leads
CREATE TABLE IF NOT EXISTS public.agency_saved_leads (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  agency_id uuid NOT NULL REFERENCES public.agency_accounts(id) ON DELETE CASCADE,
  name text NOT NULL,
  phone text,
...
DROP POLICY IF EXISTS "Agency users can view own saved leads" ON public.agency_saved_leads;
CREATE POLICY "Agency users can view own saved leads"
  ON public.agency_saved_leads FOR SELECT
  USING (agency_id IN (SELECT id FROM public.agency_accounts WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Agency users can insert own saved leads" ON public.agency_saved_leads;
CREATE POLICY "Agency users can insert own saved leads"
  ON public.agency_saved_leads FOR INSERT
  WITH CHECK (agency_id IN (SELECT id FROM public.agency_accounts WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Agency users can update own saved leads" ON public.agency_saved_leads;
CREATE POLICY "Agency users can update own saved leads"
  ON public.agency_saved_leads FOR UPDATE
  USING (agency_id IN (SELECT id FROM public.agency_accounts WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Agency users can delete own saved leads" ON public.agency_saved_leads;
CREATE POLICY "Agency users can delete own saved leads"
  ON public.agency_saved_leads FOR DELETE
  USING (agency_id IN (SELECT id FROM public.agency_accounts WHERE user_id = auth.uid()));

-- Updated_at trigger
DROP TRIGGER IF EXISTS update_agency_saved_leads_updated_at ON public.agency_saved_leads;
...
CREATE INDEX IF NOT EXISTS idx_agency_lead_searches_agency_date 
