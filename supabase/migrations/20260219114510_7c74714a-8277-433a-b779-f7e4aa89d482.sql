...
CREATE TABLE IF NOT EXISTS public.demo_phone_numbers (
  id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  owner_id uuid NOT NULL,
  owner_type text NOT NULL DEFAULT 'admin' CHECK (owner_type IN ('admin', 'agency')),
...
DROP POLICY IF EXISTS "Owners manage their demo profiles" ON public.demo_profiles;
CREATE POLICY "Owners manage their demo profiles"
  ON public.demo_profiles FOR ALL
  USING (owner_id = auth.uid())
  WITH CHECK (owner_id = auth.uid());

DROP POLICY IF EXISTS "Super admins manage all demo profiles" ON public.demo_profiles;
...
DROP POLICY IF EXISTS "Owners manage their demo phone numbers" ON public.demo_phone_numbers;
CREATE POLICY "Owners manage their demo phone numbers"
  ON public.demo_phone_numbers FOR ALL
  USING (owner_id = auth.uid())
  WITH CHECK (owner_id = auth.uid());

DROP POLICY IF EXISTS "Super admins manage all demo phone numbers" ON public.demo_phone_numbers;
...
CREATE INDEX IF NOT EXISTS idx_demo_phone_numbers_phone ON public.demo_phone_numbers(phone_e164);
CREATE INDEX IF NOT EXISTS idx_demo_profiles_owner ON public.demo_profiles(owner_id);
