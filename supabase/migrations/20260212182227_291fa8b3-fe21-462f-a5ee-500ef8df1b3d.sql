...
DROP POLICY IF EXISTS "Tenant members can view integrations" ON public.tenant_integrations;
CREATE POLICY "Tenant members can view integrations"
  ON public.tenant_integrations FOR SELECT
  USING (tenant_id IN (SELECT tenant_id FROM public.tenant_users WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Tenant members can insert integrations" ON public.tenant_integrations;
CREATE POLICY "Tenant members can insert integrations"
  ON public.tenant_integrations FOR INSERT
  WITH CHECK (tenant_id IN (SELECT tenant_id FROM public.tenant_users WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Tenant members can update integrations" ON public.tenant_integrations;
CREATE POLICY "Tenant members can update integrations"
  ON public.tenant_integrations FOR UPDATE
  USING (tenant_id IN (SELECT tenant_id FROM public.tenant_users WHERE user_id = auth.uid()));

DROP POLICY IF EXISTS "Tenant members can delete integrations" ON public.tenant_integrations;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_active_jobs_external_id
