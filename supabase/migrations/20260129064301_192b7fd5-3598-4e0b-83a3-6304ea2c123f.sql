...
DROP POLICY IF EXISTS "Tenant users can view webhook executions" ON public.webhook_executions;
...
CREATE INDEX IF NOT EXISTS idx_webhook_executions_idempotency ON public.webhook_executions(idempotency_key);
CREATE INDEX IF NOT EXISTS idx_webhook_executions_run ON public.webhook_executions(run_id);
