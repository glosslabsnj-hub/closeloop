...
-- Fix get_ai_readiness RPC to properly detect ai_booking_mode
-- The issue is the record fetch may be failing or the field check is not working
...
BEGIN
  ...
  -- Get booking mode directly with explicit null handling
  ...
  WHEN 'service' THEN
    ...
    -- Fixed: properly check for non-null and non-empty booking mode
