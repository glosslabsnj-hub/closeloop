-- Fix: Policies check in get_ai_readiness was accepting empty JSON objects ({})
-- as valid policies data. A tenant with only default payment methods (e.g. {"payment_methods":["cash"]})
-- and no cancellation/deposit policy was incorrectly marked as "policies complete".
--
-- This migration replaces the policies check to require EITHER:
-- 1. A text cancellation_policy longer than 10 chars, OR
-- 2. An ai_policies_json object that contains meaningful policy content
--    (cancellation_text, deposit info, or enough data to indicate real configuration)
...
BEGIN
  ...
  ELSIF policies_json IS NOT NULL AND jsonb_typeof(policies_json) = 'object' AND policies_json != '{}'::jsonb THEN
    -- Check for meaningful policy content beyond just default payment methods
    -- Require at least one of: cancellation text, deposit config, or refund policy
