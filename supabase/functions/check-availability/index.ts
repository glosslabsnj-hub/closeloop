...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Check if requested time is within business hours (numeric comparison to avoid string padding issues)
    const openTime = normalizedDay.open;
    const closeTime = normalizedDay.close;
    const openMin = timeToMinutes(openTime);
    const closeMin = timeToMinutes(closeTime);
    const requestedMin = timeToMinutes(requested_time);
    const requestedEndMin = requestedMin + duration_minutes + bufferMinutes;

    console.log(`Hours check: requested ${requested_time} (${requestedMin}min) end ${requestedEndMin}min vs open ${openTime} (${openMin}min) close ${closeTime} (${closeMin}min)`);
