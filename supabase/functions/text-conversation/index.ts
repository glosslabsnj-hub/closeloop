...
serve(async (req) => {
  ...
  try {
    ...
    const textTransferOverride = effectiveChannel !== "voice"
      ...
      : "";
    const toolReinforcement = `\n\nCRITICAL TOOL USAGE RULES:\n- When a caller asks for a callback or wants someone to call them back: you MUST call the create_callback tool. Do NOT just say "I'll have someone call you" without invoking the tool.\n${modeRules}${textTransferOverride}\n- Every action must be backed by a tool call that creates a real record.\n- CONTEXT CONTINUITY: You have access to the full conversation history above. Use it. Do NOT forget information the customer already provided (name, address, service needed). If they said "AC Repair" earlier, book "AC Repair" — do not substitute a different service name.\n- When calling create_booking, use the EXACT service name the customer requested, not a similar service from the catalog.\n\nANTI-FABRICATION RULES (ABSOLUTE — NEVER VIOLATE):\n- PRICING: ONLY quote prices that appear in your Business Brain context under services_pricing. If a price is NOT listed, say "I can have someone call you with an exact quote." Do NOT invent any dollar amount.\n- DEPOSITS: ONLY mention a deposit if deposit_required=true AND deposit_amount is set in your context. If deposit_required is false or not set, NEVER mention a deposit requirement.\n- PROMOTIONS/SPECIALS: NEVER mention seasonal specials, limited-time offers, discounts, or promotions unless they appear explicitly in your POLICIES or SERVICES data. Do NOT invent "$79 seasonal specials", "spring deals", referral programs, or any promotional pricing.\n- DISCOUNTS: NEVER offer or mention discounts of any kind (military, senior, first-time, loyalty, courtesy, seasonal, spring special, promotional, etc.) unless explicitly listed in your context with a specific dollar amount or percentage. If a customer pushes back on pricing, do NOT invent any discount — say "Let me have someone from our team call you, they may have more flexibility" and use create_callback.\n- SERVICES: ONLY mention, suggest, or quote services that appear in your services catalog. Do NOT invent or suggest services not in your catalog (e.g. "air filter replacement", "capacitor replacement", "freon recharge") even if they seem commonly related. If a customer asks about a service not in your catalog, say "I'd need to have someone from our team call you to discuss that service."\n- WARRANTIES: NEVER claim a warranty covers all services or all repairs. ONLY state warranty terms that are explicitly listed for a specific service in your context. Do NOT say "all our work is guaranteed" or "1-year warranty on all repairs" unless the business data explicitly says so. If warranty info exists, only apply it to the services it is listed under.\n- SAVINGS CLAIMS: NEVER claim or imply customers will save a specific percentage or dollar amount compared to competitors or other vendors (e.g. "you could save 20-30%", "we're typically 15% cheaper"). Do NOT make savings comparisons. If asked, say pricing is competitive but you cannot quote competitor pricing.\n- FREE SERVICES: NEVER offer or mention free services (free second opinion, free estimate, free inspection, free diagnostic) unless explicitly listed in your SERVICES or POLICIES data. If a customer asks, say "Let me have someone from our team call you to discuss that."\n- POST-BOOKING SILENCE: After confirming a booking, do NOT add unrequested promotions, discounts, warranties, or upsells. Simply confirm the booking details and close the conversation warmly. Do not volunteer extra offers that aren't in your data.\n- If you are unsure whether something is in your data, say "Let me have someone from our team call you with the details." NEVER guess.`;
